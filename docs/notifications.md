...
| Slack | 静态配置 | `SLACK_WEBHOOK_URL` 或 `SLACK_BOT_TOKEN` + `SLACK_CHANNEL_ID` | `SLACK_MRKDWN_CONVERT` | Bot 优先用于文本与图片同频道发送 |
...
- `SLACK_MRKDWN_CONVERT` 控制是否在发送前将标准 Markdown 自动转换为 Slack mrkdwn 方言（默认 `true`）。若上游内容已是 mrkdwn 格式，设为 `false` 可跳过转换。依赖 `markdown-to-mrkdwn>=0.3.3`，缺失时自动降级为原始内容发送并记录一次警告。
